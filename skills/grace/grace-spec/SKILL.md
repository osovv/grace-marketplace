...
3. Create a deterministic `C-*` change id from the requested change. Use uppercase kebab form.
4. Write `spec.xml` from `references/change-spec-template.xml` with exactly one direct `C-*` wrapper.
5. If rationale, alternatives, scenarios, or external constraints would otherwise bloat the spec, write `design-context.xml` from `references/design-context-template.xml`. Mark it non-normative in the content.
6. If approval is not explicit, leave `spec.xml` as `status="draft"` and report the approval step needed.
</workflow>

<hard_rules>
- `spec.xml` is the source of truth for `grace-plan`.
- `design-context.xml` is optional explanatory memory and never adds requirements beyond `spec.xml`.
- Do not implement code, change graph/verification current state, or create retroactive change bundles.
- Semantic anchors must be XML tags, never attributes.
- Recommend `grace lint --path <project-root>` after writing or updating the change bundle.
</hard_rules>
</skill>
